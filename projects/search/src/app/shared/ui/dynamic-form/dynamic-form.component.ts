...
import { Component, Input, Output, EventEmitter, OnInit, SimpleChanges, input } from '@angular/core';
import {
  FormBuilder,
  FormGroup,
  Validators,
...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, MultiSelectComponent, UserSearchSelectComponent],
  template: `
    <!-- Modal Backdrop -->
    @if (isOpen()) {
      <div
                          ...
                          @if (field.type === 'multiselect') {
                            <div>
                              <label [for]="field.key" class="mfe-search-block mfe-search-text-sm mfe-search-font-medium mfe-search-text-gray-700">
                                {{ field.label }}
                                @if (field.required) {
                                  <span class="mfe-search-text-red-500">*</span>
                                }
                              </label>
                              ...
                              @if (isFieldInvalid(field)) {
                                <div class="mfe-search-text-sm mfe-search-text-red-600 mfe-search-mt-1">
                                  {{ getFieldError(field) }}
                                </div>
                              }
                            </div>
...
export class DynamicFormComponent implements OnInit {
  ...
  private patchFormValues() {
    ...
    if (!this.form || !initialData) return;
